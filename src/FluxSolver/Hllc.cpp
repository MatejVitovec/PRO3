...
Vars<5> Hllc::claculateFlux(const Primitive& ul, const Primitive& ur, const Vars<3>& normalVector) const
{    
    double rhoL = ul.density();
    double pL = ul.pressure();
    double nuL = ul.normalVelocity(normalVector);
    double aL = ul.soundSpeed();

    double rhoR = ur.density();
    double pR = ur.pressure();
    double nuR = ur.normalVelocity(normalVector);
    double aR = ur.soundSpeed();
    ...
    {
        //left state
        return ul.conservativeFlux(normalVector);
    }
    else if (sr <= 0)
    {
        //right state
        return ur.conservativeFlux(normalVector);
    }
    else if (sm >= 0)
    {
        //middle-left state

        double uL = ul.velocityU();
        double vL = ul.velocityV();
        double wL = ul.velocityW();
        double EL = ul.totalEnergy();
    ...
    else
    {
        ...
        double uR = ur.velocityU();
        double vR = ur.velocityV();
        double wR = ur.velocityW();
        double ER = ur.totalEnergy();
