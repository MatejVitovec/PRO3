...
Vars<3> Iawps95::updateThermo(const Compressible& data) const
{
...
Compressible Iawps95::primitiveToConservative(const Vars<5>& primitive) const
{
...
Compressible Iawps95::isentropicInlet(double pTot, double TTot, Vars<3> velocityDirection, Compressible stateIn) const
{
    ...
    while (change > numericalTolerance)
    {
        ...
        double auxConst = critRho*critT*(deltaOld/tauOld);

        double f = s(rhoTot, TTot) - specGasConst*(tauOld*(phi0t(deltaOld, tauOld) + phirt(deltaOld, tauOld)) - phi0(deltaOld, tauOld) - phir(deltaOld, tauOld));
        ...
        double gd = -critRho*(critT/tauOld) - 2.0*auxConst*auxPhird - auxConst*deltaOld*phirdd(deltaOld, tauOld);
        double gt = auxConst/tauOld + auxConst*(deltaOld/tauOld)*auxPhird - auxConst*delta*auxPhirdt;
...
double Iawps95::a2(double rho, double T) const
{
    double delta = rho/critRho;
    double tau = critT/T;
...
double Iawps95::a(double rho, double T) const
{
