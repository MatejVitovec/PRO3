...
std::vector<std::shared_ptr<BoundaryCondition>> createBoundaryCondition(const Mesh& mesh)
{
	...
	for (int i = 0; i < boundaryTypes.size(); i++)
	{
		...
		{
			//out.push_back(std::make_shared<PressureTemperatureInlet>(aux, 99980.0, 422.65, Vars<3>({1.0, 0.0, 0.0})));
			out.push_back(std::make_shared<PressureTemperatureInlet>(aux, 98071.7, 298.65, vector3toVars(angleAngleToUnit(19.3, 0.0))));
		}
		else if(boundaryTypes[i] == BoundaryCondition::PRESSUREDENSITYINLET)
		{
			out.push_back(std::make_shared<PressureDensityInlet>(aux, 1.0/1.4, 1.0, Vars<3>({1.0, 0.0, 0.0})));
		}
		else if(boundaryTypes[i] == BoundaryCondition::PRESSUREOUTLET)
		{
			//out.push_back(std::make_shared<PressureOutlet>(aux, /*0.5264*/80000.0));
			out.push_back(std::make_shared<PressureOutlet>(aux, 40548.109));
		}
		else if(boundaryTypes[i] == BoundaryCondition::FREEBOUNDARY)
		{
			out.push_back(std::make_shared<FreeBoundary>(aux));
		}
		else if(boundaryTypes[i] == BoundaryCondition::WALL)
		{
			out.push_back(std::make_shared<Wall>(aux));
		}
		else if(boundaryTypes[i] == BoundaryCondition::PERIODICITY)
		{
			out.push_back(std::make_shared<Periodicity>(aux, Vector3(0.0, 0.0551168, 0.0), "aaa", mesh));
		}
		else if(boundaryTypes[i] == 100)
		{
			out.push_back(std::make_shared<Periodicity>(aux, Vector3(0.0, -0.0551168, 0.0), "bbb", mesh));
		}
	}
